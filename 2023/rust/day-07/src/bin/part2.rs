...
impl Hand {
    fn r#type(&self) -> Type {
        let mut counts = self.cards.iter().fold(HashMap::new(), |mut count, card| {
            count.entry(card.0).and_modify(|c| *c += 1).or_insert(1);
            count
        });

        let j = counts.remove(&'J').unwrap_or(0);

        let mut counts = counts.values().cloned().collect::<Vec<i32>>();
        counts.sort();
        counts.reverse();

        if let Some(first) = counts.get_mut(0) {
            *first += j;
        } else {
            counts.push(5);
        }
